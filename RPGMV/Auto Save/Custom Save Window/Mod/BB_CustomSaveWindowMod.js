...
Imported.BB_CustomSaveWindowMod = true;

(function () {
  'use strict';

  // Zero_G Config
  // Add frame and a title to bottom right item (time)
  var addTitleToTime = true;

  // Snap frame method
  // Method1: Make a frame extending 10px from the snap, not bad, but method2 looks better
  ...
  // プラグインパラメータ管理
  //-----------------------------------------------------------------------------
  var parameters = PluginManager.parameters('BB_CustomSaveWindowMod');
  ...
  Window_SavefileList.prototype.drawItem = function (index) {
    ...
    if (BBSWMaxItem >= 1) {
      // Zero_G mod: Resize frame for time, item 1 (bottom right). As we are hiding the title and
      // only leaving the time, frame needs to be modified

      // Remove left frame
      //this.contents.fillRect(rect.x + 490, rect.y + 70, BBSWTF1, 26, this.textColor(BBSWOutFC));

      // fillRect(xPosition, yPosition, width, height)
      this.contents.fillRect(rect.x + 675, rect.y + 68, 91, 2, this.textColor(BBSWOutFC)); //top bar
      this.contents.fillRect(rect.x + 675, rect.y + 96, 91, 2, this.textColor(BBSWOutFC)); //bottom bar
      this.contents.fillRect(rect.x + 764, rect.y + 70, 2, 26, this.textColor(BBSWOutFC)); //right bar
      this.contents.fillRect(rect.x + 675, rect.y + 70, 2, 26, this.textColor(BBSWOutFC)); //left bar

      if (addTitleToTime) {
        // Add time title frame (using title of item2[top-right] moved to the right)
        this.contents.fillRect(rect.x + 675, rect.y + 37, 91, 26, this.textColor(BBSWOutFC));
        // Search for BBSWItem2T to see the line where the title is drawn
      }
    }
    if (BBSWMaxItem >= 2) {
      // Zero_G mod: disable frame from item 2 (top right)
    ...
    // Zero_G: Snap (at the end of the rest of the code so the snap can be at top of the frames)
    if (info) {
      let snapRect = this.itemRectForText(index);
      this.drawSnappedImage(info, snapRect, valid);
    }
  ...
  Window_SavefileList.prototype.drawContents = function (info, rect, valid) {
    ...
    if (y2 >= lineHeight) {
      ...
      //-----------------------------------------------------------------------------
      //項目2 右上-------------------------------------------------------------------
      //Zero_G change drawing of item2 title to the where the value used to be, so
      //it acts as a title of item 1
      //this.drawText(BBSWItem2T, rect.x + 495, y2 - 34, BBSWTF2 - 10);
      if (addTitleToTime) this.drawText(BBSWItem2T, rect.x + 680, y2 - 34, BBSWTF2 - 10);
  ...
  // Compatibility with Felski Autosave (Felski must be loaded first)
  var _Window_SavefileList_prototype_drawFileId = Window_SavefileList.prototype.drawFileId;
  Window_SavefileList.prototype.drawFileId = function (id, x, y) {
    if(Imported.FELSKI_AUTOSAVE){
      //_Window_SavefileList_prototype_drawFileId
      if(id <= Felski.AUTOSAVE.slots){
        if (this._mode === 'save') {
            this.changePaintOpacity(false);
        }
        this.drawText(Felski.AUTOSAVE.autosaveText + ' ' + id, x, y, 120);
      }else{
          this.changePaintOpacity(true);
          this.drawText(TextManager.file + ' ' + (id - Felski.AUTOSAVE.slots), x, y, 120);
      }
    } else{
  ...
  /*------------------------------------------------------------------------------------*/
  /*  ZERO_G MOD: Integrate SaveWithSnap                                                */
  /*------------------------------------------------------------------------------------*/

  //-----------------------------------------------------------------------------
  // KMS_AccelerateFileScene
  ...
  DataManager.makeSavefileBitmap = function () {
    ...
    var scale = 0.15; // This used to be a param
