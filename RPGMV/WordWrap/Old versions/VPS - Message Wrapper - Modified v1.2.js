...
(function($){
		...
		var maxWidth = 40;							// Max width of textbox with face
		var maxWidthWithoutFace = 50;		// Max width of textbox without face
		...
		const whiteSpace = false;       // Wordwrap without handling names (previous needs to be set to false
    ...
    Window_Message.prototype.wordWrapConvertWhiteSpace = function(text) {
    	...
    	if ($gameMessage._faceName == ""){
	    	...
	    	if (splittedText[0] != ""){
	    		...
	    		text = Soulpour.soulpour_wordWrapper(splittedText[1], wordWrapWidth, "\n");
	    	}
    	}else{
    		if(whiteSpace){
    			text = text.replace(/\n/g, " ");
