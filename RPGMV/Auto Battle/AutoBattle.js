...
(function(){
	...
	var autoBattleUsableMessage = String(parameters['Auto Battle Usable Message'] || 'End auto battle with Shift');
	var autoBattleMessage = String(parameters['Auto Battle Message'] || 'Shift for auto battle');
			...
			if (actor && actor.canInput()){
				...
				if (autoBattleUseSkill){
					...
					actor._actions.forEach(function(action){
						action.setAttack();
						action.setTarget(0); // 龍尾追加(攻撃対象を先頭の敵に固定化)
	...
	// 龍尾変更(戦闘強制中断時にオートバトルOFF化)
//		if (BattleManager.isAborting() || $gameParty.members().length === 0 ||
		if (BattleManager.isAborting() || BattleManager.isBattleEnd() || $gameParty.members().length === 0 ||
