 ...
 * @help
 ...
 Must use a modified clipboard_illule plugin.

 Script has an option to store translation in a file, and read upon that file
 ...
 1.12   -Ditched plugin parameters, replaced for manual configuration variables
        ...
        -Handle hearths ♥ ♡ in text better (code in clipboard_illule also changed)
...
(function ($) {
 ...
 // neccesary if skipping text with ctrl button, and clipboard_illule is disables while skipping, 
 // only in modded version). 
 // Will display a message at the top rigth screen.
 // >Default: g
 $.autoInsertToogleKey = 'g';

 // Auto replace choices with transalted ones. Works 99% of the time. But if it fails, use
 // the copyChoicesButton, and it will send all choices and text for translation, it will
 ...
 // Pre translation replacements in clipboard_illule.
 // Useful when a replacement before translation isn't working.
 // Note: script with replace # with a heath symbol.
 ...
 /*--------------------------------------------------------------------------------------*/
  ...
  $.escapeText = true;
  $.replacingChoicesStopIlule = false; // used to stop Illule when replacing choices
  ...
  Window_Message.prototype.update = function () {
    ...
    if(this.isOpen() && !startChoiceReplaceNormal && !startChoiceReplaceStored){ // Check if message window is opened
      ...
      if(this.isOpen() && translationSent && $.useTranslationCache && !cacheOverwrite){ // Check if message window is opened
        clipboardText = clipboard.get('text'); // Gets clipbard_illule jp text, not the translated one
        storedTranslations = readFile('translationsCache') || storedTranslations;
        ...
        for(const japText in storedTranslations){
          if(clipboardText == japText){
            ...
            if(skipCachedText){ 
              ...
              setTimeout(() => { // timeout, give time for illule to copy next text window
                this.pause = false; 
                this.terminateMessage();
              }, 400);
              break;
  ...
  // Add normal parenthesis when flag from clipboard_illule was set
