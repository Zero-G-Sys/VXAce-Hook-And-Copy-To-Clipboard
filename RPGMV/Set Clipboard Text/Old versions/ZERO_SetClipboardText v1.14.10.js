 ...
 /*--------------------------------------------------------------------------------------*/
  ...
  Window_Selectable.prototype.update = function() {
    ...
    if (startChoiceReplaceNormal) {
      ...
      && clipboardText.localeCompare('') != 0){
        ...
        // Restore heart characters (Can't use color codes)
        clipboardText = clipboardText.replace(/#/g, '♥'); 
        clipboardText = clipboardText.replace(/%23/g, '♥');
  ...
  Window_Message.prototype.update = function () {
    ...
    if (windowTextWithChoices){
      ...
      //by clipboardlulle block ignore. That won't trigger here, so we need to remove them here.
    ...
    if(this.isOpen() && !startChoiceReplaceNormal && !startChoiceReplaceStored){ // Check if message window is opened
      ...
      if(this.isOpen() && translationSent && !cacheOverwrite && processCache){ // Check if message window is opened
        ...
        clipboardText = LastMemTextSend || clipboard.get('text'); // Gets clipbard_llule jp text, not the translated one
