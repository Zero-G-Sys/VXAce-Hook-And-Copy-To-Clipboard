...
(function ($) {
 ...
 /*--------------------------------------------------------------------------------------*/
  ...
  Window_Message.prototype.startMessage = function() {
    ...
    // Handle auto replace choices
    if($gameMessage.isChoice() && $.autoReplaceChoices) preProcessChoiceReplace();
  ...
  function preProcessChoiceReplace(){
    ...
    $.escapeText = false; // prevent clipboard_illule to capture text
  ...
  Window_ChoiceList.prototype.drawItem = function(index, override = false) {
    ...
    if(override) this.drawTextEx(translatedChoices[index].trim(), rect.x, rect.y); // draw translated choices
  ...
  Window_Selectable.prototype.update = function() {
    if (startChoiceReplace) {
      ...
      && clipboardText.localeCompare('') != 0){
        ...
        if($gameMessage._texts.length){ // If there is a text window translate text and replace it
          let text = $gameMessage._texts.join(' ');
          clipboard.set(text, 'text');
          textOverflowed = false;
          $.escapeText = true;
        }    
