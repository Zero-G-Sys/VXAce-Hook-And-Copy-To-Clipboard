...
(function ($) {
 ...
 const removeNameFromBattleText = true;
 ...
 /*--------------------------------------------------------------------------------------*/
  ...
  Window_Message.prototype.startMessage = function() {
    ...
    if(clipboardDisabledBattle){ // ClipboardIllule disabled during battle, send text manually
      ...
      if(text !== '' && /[一-龠]+|[ぁ-ゔ]+|[ァ-ヴー]+|[ａ-ｚＡ-Ｚ０-９]+|[々〆〤！？]+/.test(text)){  // Check that text is in JP
        ...
        LastMemTextSend = text; // This var is normally filled by ClipboardLulle, but it's used when storing translation cache, as we are overriding clipboardLulle, we need to set it with jp text
  ...
  function preProcessChoiceReplace(){
    ...
    if($.ignoreDeeplIfCachedText && storedTranslationExist){
      text = $.ignoreDeeplIfCachedTextCharacter + text;
    }
