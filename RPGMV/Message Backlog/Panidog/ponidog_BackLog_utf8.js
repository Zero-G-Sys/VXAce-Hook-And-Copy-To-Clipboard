(function () {
  ...
  var bBackLogNameOn = Boolean(
    parameters['bBackLogNameOn_forYepMessageCore'] || 0
  );
  ...
  Game_Message.prototype.add = function (text) {
    ...
    if (nNameSerch >= 0) {
      ...
      if (sNameSet.match('nickname=')) {
        ...
        sNameSet =
          $gameActors.actor(parseInt(_nActor[1])).nickname() +
          $gameActors.actor(parseInt(_nActor[1])).name();
      } //if(sNameSet.match("nickname=")){
    } //if(nNameSerch>=0){

    //スクリプトコマンドで名前指定があればそちらを優先する。
    if (oBackLog.sNameBoxSet_ponidogBackLog.length > 0)
      sNameSet = oBackLog.sNameBoxSet_ponidogBackLog;
  ...
  BackLog.prototype.BackLog_Call = function () {
    ...
    if (sBackLog.length > 0) {
      ...
      if (nStart <= 0 || nStart == null) {
        ...
        //ページ遡りの上限
        nBackLogPage =
          sBackLog.length % nBackLogLineNum == 0
            ? parseInt(sBackLog.length / nBackLogLineNum)
            : parseInt(sBackLog.length / nBackLogLineNum) + 1;
  ...
  Buck_logSp.prototype.redraw = function () {
    ...
    for (i = 0; i <= nLast; i++) {
      ...
      this.bitmap.drawText(
        text,
        10,
        10 - height / 2 + 0.7 * this.bitmap.fontSize + this.bitmap.fontSize * i,
        width,
        height,
        'Left'
      );
  ...
  var _Spriteset_Base_prototype_createUpperLayer =
  ...
  var _Spriteset_Base_prototype_createSpriteset =
    Spriteset_Base.prototype.createSpriteset;
  ...
  Window_Message.prototype.updateInput = function () {
    ...
    if (TouchInput.isTriggered()) {
      ...
      if (
        oBackLog.ButtonPress(nBackLogButtonDownId) ||
        oBackLog.ButtonPress(nBackLogButtonUpId)
      ) {
        if (oBackLog.ButtonPress(nBackLogButtonDownId))
          oBackLog._nBackLogPage--;
    ...
    if (this.pause) {
      //backlog-call
      if (
        (Input.isTriggered('pageup') && !oBackLog._BackLogBusy) ||
        (TouchInput._wheelY < 0 && !oBackLog._BackLogBusy)
      ) {
        Input.update();
        oBackLog._nBackLogPage++;
        oBackLog.BackLog_Call();
  ...
  BackLog.prototype.ButtonPress = function (picID) {
    ...
    var w = parseInt(
      SceneManager._scene._spriteset._pictureContainer.children[nPictureId]
        .width
    ); //
    var h = parseInt(
      SceneManager._scene._spriteset._pictureContainer.children[nPictureId]
    ...
    var nButton_w = parseInt(
      (nButtonW * $gameScreen.picture(picID)._scaleX) / 100
    );
    var nButton_h = parseInt(
      (nButtonH * $gameScreen.picture(picID)._scaleY) / 100
    );
    ...
    return (
      tx >= nButton_X &&
      tx <= nButton_X + nButton_w &&
      ty >= nButton_Y &&
      ty <= nButton_Y + nButton_h
    );
  };
})();
