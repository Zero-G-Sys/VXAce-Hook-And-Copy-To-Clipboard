...
(function ($) {
  ...
  var maxNamesLenght = 15;
  var nameInSameLine = true; // Is name and text continous (true) or separed by a line break (false)?
  ...
  */
  function replaceNames(text){
    ...
    for (const [key, value] of Object.entries(replacementsCombined)) {
      ...
      let name = text.substring(text.indexOf(']')+1) // Separate name from color code and match that
      name = name.replace('', '');
