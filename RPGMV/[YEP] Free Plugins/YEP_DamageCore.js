...
////ダメージの結果をオリジナルのスクリプトで弄る用
Game_Action.prototype.mu_keisann = function (value, target) {
    var item = this.item();
    var a = this.subject();
    var b = target;
    ...
    //疲弊と幽霊恐怖症と酔はダメージを比率でへらす
    if (a.isStateAffected(35) && value > 0) {//疲弊
        value /= 4;
    }
    if (a.isStateAffected(34) && value > 0) {//幽霊恐怖症
        value /= 2;
    }
    if (a.isStateAffected(82) && value > 0) {//酔
      value /= 2;
  }
 
    
    if (item.note.match(/<ヘッドオン>/)) {
        var hanekaeri = Math.round(1 * (value / 5.0));
        this.subject()._hp -= hanekaeri;
        if (this.subject()._hp < 1) this.subject()._hp = 1;
    }



    //難易度　なお変数が0スタンダードの場合は補正がないので何もしない
    if (value > 0) {
        if ($gameVariables.value(20) == 1) {//カジュアル
            if (target.constructor === Game_Enemy) {
                value *= 1;
            } else {
                value *= 0.5;
            }
        ...
        else if ($gameVariables.value(20) == 2) {//チート
            if (target.constructor === Game_Enemy) {
                value *= 100;
            } else {
                value *= 0.01;
        ...
        //    console.log("B");
        } else if ($gameVariables.value(20) == 3) {//ヒロイック
            if (target.constructor === Game_Enemy) {
                value *= 0.75;
            } else {
                value *= 1.2;
            }
        } else if ($gameVariables.value(20) == 4) {//探索者の墓場
            if (target.constructor === Game_Enemy) {
                value *= 0.5;
            } else {
                value *= 1.5;
    ...
    if (target.constructor === Game_Enemy && value > 0) {
        if ($gameVariables._data[21] == null) {
            $gameVariables._data[21] = 0;
        }
        $gameVariables._data[21] += Math.round(value);
    }
...
Game_Action.prototype.evalDamageFormula = function(target) {
    try {
        ...
        value = this.mu_keisann(value, target);
